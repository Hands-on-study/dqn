...
- 에이전트: DQNAgent 클래스
  ...
  - 리플레이 메모리: deque를 사용해 경험(상태, 행동, 보상, 다음 상태, 종료 여부)을 저장
  - 타깃 네트워크: 안정적 학습을 위해 별도의 target_model 사용
  - ε-greedy 정책: 탐험(랜덤 행동)과 활용(Q-value 기반 행동)을 조절
  - 훈련 루프:
    1. 환경 reset 후 반복   
    2. 행동 선택 (ε-greedy)   
    3. 보상 shaping (실패 시 -100)
    4. 경험 저장
    5. 메모리 충분히 쌓이면 미니배치 학습
    6. 일정 에피소드마다 타깃 네트워크 업데이트
    7. 평균 점수가 기준 이상이면 학습 종료 및 모델 저장   
